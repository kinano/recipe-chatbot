...
class RecipeRetriever:
    ...
    def get_recipe_rank(
        self, query: str, target_recipe_id: int, top_k: int = 100
    ) -> Optional[int]:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "avg_ingredients": sum(r.get("n_ingredients", 0) for r in self.recipes)
            / len(self.recipes),
            "avg_steps": sum(r.get("n_steps", 0) for r in self.recipes)
            / len(self.recipes),
            "avg_cooking_time": sum(
                r.get("minutes", 0) for r in self.recipes if r.get("minutes", 0) > 0
            )
            / len([r for r in self.recipes if r.get("minutes", 0) > 0]),
        }
...
def create_retriever(
    recipes_path: Path, index_path: Optional[Path] = None, rebuild_index: bool = False
) -> RecipeRetriever:
    """
...
def retrieve_bm25(
    query: str, corpus: List[Dict[str, Any]], top_n: int = 5
) -> List[Dict[str, Any]]:
    """
