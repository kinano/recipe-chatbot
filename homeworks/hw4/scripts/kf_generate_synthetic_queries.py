...
def analyze_recipe_complexity(recipes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for recipe in recipes:
        ...
        for ing in ingredients:
            ing_lower = ing.lower()
            if any(term in ing_lower for term in [
                'kombu', 'seaweed', 'cardamom', 'saffron', 'truffle', 'duck', 'lamb', 
                'venison', 'xanthan', 'agar', 'tahini', 'miso', 'coconut milk', 
                'curry paste', 'fish sauce', 'worcestershire', 'anchovy', 'crab',
                'black truffle', 'amish starter'
            ]):
        ...
        for step in steps[:15]:
            ...
            techniques = [
                'confit', 'sous vide', 'flambé', 'julienne', 'brunoise', 
                'emulsify', 'tempering', 'proving', 'roux', 'reduction',
                'caramelize', 'deglaze', 'fold', 'whip', 'roast', 'braise'
            ]
        ...
        cultural_tags = []
        for tag in tags:
            if any(culture in tag.lower() for culture in [
                'french', 'italian', 'asian', 'indian', 'mexican', 'thai', 
                'moroccan', 'amish', 'cajun', 'chinese', 'japanese', 'korean'
            ]):
        ...
        if salient_facts:
            complex_recipes.append({
                ...
                'complexity_score': len(salient_facts) + (len(ingredients)/10) + (minutes/100)
            })
...
def generate_knowledge_intensive_queries(
    complex_recipes: List[Dict[str, Any]], 
    num_queries: int = 100
    ...
    for recipe in complex_recipes:
        if recipe['specialty_ingredients']:
            ...
            for ing in recipe['specialty_ingredients'][:2]:
                ing_queries = [
                    f"recipes using {ing}",
    ...
    for recipe in complex_recipes[:8]:
        recipe_id = recipe['id']
        name = recipe['name']
        words = name.split()
    ...
    for recipe in complex_recipes[:5]:
        recipe_id = recipe['id']
...
def save_queries(queries: List[str], query_metadata: List[Dict[str, Any]], output_path: str):
    ...
    print(f"Generated {len(queries)} knowledge-intensive queries saved to {output_path}")
...
def main():
    ...
    queries, query_metadata = generate_knowledge_intensive_queries(complex_recipes, args.num_queries)
